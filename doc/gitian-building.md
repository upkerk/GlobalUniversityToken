...
*Setup instructions for a Gitian build of Globaluniversitytoken Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Globaluniversitytoken
...
- [Building Globaluniversitytoken Core](#building-globaluniversitytoken-core)
...
Clone the git repositories for globaluniversitytoken and Gitian.
...
in the globaluniversitytoken repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Globaluniversitytoken Core
----------------

To build Globaluniversitytoken Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the globaluniversitytoken repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/globaluniversitytoken/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/upkerk/globaluniversitytoken
...
For example:
```bash
URL=https://github.com/thrasher-/globaluniversitytoken.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit globaluniversitytoken=${COMMIT} --url globaluniversitytoken=${URL} ../globaluniversitytoken/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit globaluniversitytoken=${COMMIT} --url globaluniversitytoken=${URL} ../globaluniversitytoken/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit globaluniversitytoken=${COMMIT} --url globaluniversitytoken=${URL} ../globaluniversitytoken/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the globaluniversitytoken git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../globaluniversitytoken/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/upkerk/globaluniversitytoken-detached-sigs.git

BTCPATH=/some/root/path/globaluniversitytoken
SIGPATH=/some/root/path/globaluniversitytoken-detached-sigs

./bin/gbuild --url globaluniversitytoken=${BTCPATH},signature=${SIGPATH} ../globaluniversitytoken/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/globaluniversitytoken-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/globaluniversitytoken-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/globaluniversitytoken-osx-build.assert
```
...
[upkerk/gitian.sigs.gut](https://github.com/upkerk/gitian.sigs.gut/) repository, or if that's not possible create a pull
